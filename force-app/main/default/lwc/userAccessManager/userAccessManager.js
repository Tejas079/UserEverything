...
export default class UserPermMetadata extends LightningElement {
   /* @track showWelcomeModal = true;

    connectedCallback() {
        // Check localStorage for user preference
        const dontShowAgain = localStorage.getItem('dontShowWelcome');
        this.showWelcomeModal = !dontShowAgain;
    }
   ...
   // _showSystemPermissions = false;
    ...
    async handleUserChange(event) {
        ...
        this.fieldPageData = []; // Reset existing data
      ...
      //  await this.loadUserPermissions();
        if(this.showSharingRules) await this.loadSharingRules();
        if(this.showRoleHierarchy) await this.loadRoleHierarchy();
    }
